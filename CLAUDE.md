...
handlers/
  ...
  memory.py             ← 메모리(DRAM/NAND) 현물가 브리핑 (send_memory_briefing)
...
services/
  ...
  semi.py               ← 반도체 업황 데이터
  memory_spot.py        ← DRAM/NAND 현물가(DRAMeXchange 스크레이핑) + 메모리주 프록시
  kr_index.py           ← 국내 지수(코스피/코스닥) 네이버 금융 폴백 조회
  macro.py              ← 매크로 일정(FOMC·CPI·고용보고서) 정적 테이블 — 모닝 시황에 첨부
...
| `/메모리`, `/memory` | 메모리(DRAM/NAND) 현물가 브리핑 |
| `/report 종목명`, `/리포트 종목명` | 종합 리포트 |
| `/오늘리포트` | 오늘 발행 증권사 리포트 |
| `/재무 종목명` | DART 재무 분석 + 차트 |
| `/us 종목명` | 미국 종목 리포트 + 차트 |
| `/공시 종목명` | 최근 DART 공시 3건 |
| `/watch`, `/unwatch`, `/watchlist` | 관심종목 관리 |
| `/거래`, `/buy`, `/sell`, `/portfolio`(`/포폴`), `/티커수정` | 포트폴리오 관리 |
| `/알림`, `/alert`, `/alerts`, `/unalert` | 가격·이벤트 알림 |
| `/earnings`, `/어닝` | 어닝 캘린더 |
| `/장보기`, `/나들이` | 생활 브리핑 |
| `/채널추가`, `/채널삭제`, `/채널목록` | 채널 모니터링 |
| `/포워딩대상`, `/포워딩추가`, `/포워딩삭제`, `/포워딩목록`, `/포워딩중복` | 채널 포워딩 (`/포워딩중복 켜기·끄기`=중복 자료 거르기) |
| `/keywords` | DART 알림 키워드 확인 |
| `/id` | 현재 채팅의 숫자 ID 확인 (포워딩 대상 설정용, 어디서나 응답) |
| `/help`, `/도움말` | 전체 명령어 도움말 |
...
| 화~금 08:00 | 메모리 현물가 (`send_memory_briefing`) — 월은 반도체가 커버해 제외 |
...
| 매 2분 | 외부 데드맨 스위치 ping (`_heartbeat`) — `HEALTHCHECK_URL` 설정 시에만 활성 |
...
- 봇 메시지 핸들러에서 `MY_TELEGRAM_ID` 체크로 본인만 응답 (`/id` 는 예외 — 어디서나 응답)
- **발송 대상 분리**: broadcast 성격(시황·섹터·반도체·메모리·오늘리포트·DART 공시 감지)은 `BROADCAST_ID` 로, 개인 응답(차트·종합리포트·포트폴리오·알림·어닝·생활 브리핑)은 `MY_TELEGRAM_ID`(DM)로 보낸다. `BROADCAST_ID = SHARED_CHANNEL_ID or MY_TELEGRAM_ID` — 공유 채널을 설정하면 채널로, 아니면 본인 DM으로 폴백. ⚠️ config.example.py 주석은 장보기·나들이도 broadcast로 적었으나 실제 `life.py`는 `MY_TELEGRAM_ID`(DM)로 발송한다(코드 기준)

## 필수 환경변수 (config.py / .env)
```
TELEGRAM_API_ID, TELEGRAM_API_HASH   ← Telethon user_client
TELEGRAM_BOT_TOKEN                   ← 봇 클라이언트
MY_TELEGRAM_ID                       ← 응답·개인 DM 대상 계정 ID
SHARED_CHANNEL_ID                    ← (선택) 공유 채널 ID(음수). broadcast 발송 대상. 미설정 시 DM 폴백
GEMINI_API_KEY                       ← Gemini API
DART_API_KEY                         ← 금감원 DART 오픈API
SENTRY_DSN                           ← (선택) Sentry 에러 트래킹
SENTRY_ENV                           ← (선택) Sentry environment 태그 (기본 production)
CHANNEL_SUMMARY_ENABLED              ← (선택) 채널 요약 서비스 활성화 (utils가 env 직접 판독)
HEALTHCHECK_URL                      ← (선택) 외부 데드맨 스위치 ping URL (2분 주기 heartbeat)
WATCH_CHANNELS                       ← (선택) 모니터링 채널 목록 (쉼표 구분, @유저네임)
TELEGRAM_USER_SESSION / TELEGRAM_BOT_SESSION  ← (선택) Telethon StringSession (파일 세션 대체)
```
